 ...
 */
package org.eclipse.ditto.policies.service;
 ...
 * pluggable-persistence switchability proof (layer 1), applied to the policies service. Symmetric to the
 * connectivity and things boundary tests.
 * <p>
 * The policies service persists through the backend-neutral persistence-API. The refactor routed the
 ...
 * {@code AbstractPolicyMongoEventAdapter} / {@code DefaultPolicyMongoEventAdapter}) or
 ...
 * <p>
 * The policies service has no out-of-scope raw-Mongo machinery, so - unlike connectivity - it needs no whitelist.
...
public final class PersistenceBoundaryArchTest {
    ...
    private static JavaClasses policiesServiceMainClasses;

    @BeforeClass
    public static void importMainClasses() {
        policiesServiceMainClasses = new ClassFileImporter()
                .withImportOption(new ImportOption.DoNotIncludeTests())
                .importPackages("org.eclipse.ditto.policies.service");
    }

    /**
     * Rule 1: no policies-service MAIN class may depend on a SWITCHABLE Mongo artifact.
     */
    @Test
    public void policiesMustNotDependOnSwitchableMongo() {
        noClasses()
                .should().dependOnClassesThat(IS_SWITCHABLE_MONGO)
                .as("policies-service MAIN code must not depend on any switchable Mongo artifact "
                ...
                .check(policiesServiceMainClasses);
    }

    /**
     * Rule 2: no policies-service MAIN class may depend on a Postgres type.
     */
    @Test
    public void policiesMustNotDependOnPostgres() {
        noClasses()
                .should().dependOnClassesThat(IS_POSTGRES)
                .as("policies-service MAIN code must not depend on any Postgres type")
