...
/**
 * Pure, I/O-free refuse-to-boot decision logic for the schema bootstrap. Kept separate from {@link PostgresSchemaManager}
...
public final class SchemaVerifier {
     ...
     *
     * @param expectedChecksum the current code checksum ({@link SchemaChecksum#current()}).
     * @param currentVersion the current code schema version ({@link PostgresSchema#VERSION}).
     ...
     * {@link #verifyTable(PostgresSchema.TableContract, String, Map, Map)} to additionally catch collation drift on the
     ...
     */
    public static void verifyTable(final PostgresSchema.TableContract contract,
    ...
    public static void verifyTable(final PostgresSchema.TableContract contract,
        ...
        for (final String collatedColumn : contract.collatedColumns()) {
            ...
            if (actualCollation == null || !actualCollation.equals(PostgresSchema.PID_COLLATION)) {
                throw new SchemaBootException(
                        "Refusing to boot: table '" + contract.tableName() + "' column '" + collatedColumn
                                + "' has collation_name '" + actualCollation + "' but expected '"
                                + PostgresSchema.PID_COLLATION + "'. A pre-existing table whose persistence-id column "
                                + "lacks COLLATE \"" + PostgresSchema.PID_COLLATION + "\" breaks pid cursor pagination "
     ...
     *
     * @param contracts the canonical contracts ({@link PostgresSchema#tableContracts()}).
     ...
     */
    public static void verifyAllTables(final List<PostgresSchema.TableContract> contracts,
            final Map<String, String> actualPrimaryKeyDefs,
            final Map<String, Map<String, String>> actualColumnsByTable) {
        for (final PostgresSchema.TableContract contract : contracts) {
    ...
    private static void verifyPrimaryKeyAndColumns(final PostgresSchema.TableContract contract,
            @Nullable final String actualPrimaryKeyDef,
            final Map<String, String> actualColumns) {
